...
\usepackage[]{amssymb} %gives us the character \varnothing
	...
	\begin{document}
		...
		\noindent
		\textbf{4.} Let $E$ be a nonempty subset of an ordered set; suppose $\alpha$ is a lower bound of $E$ and $\beta$ is an upper bound of $E$.  Prove that $\alpha \le \beta$.
		
		
		\noindent
		\textbf{5.} Let $A$ be a nonempty set of real numbers which is bounded below.  Let $-A$ be the set of all numbers $-x$, where $x \in A$.  Prove that
		\[
		\inf A = -\sup(-A).
		\]
	...
	\clearpage
	\noindent
	\textbf{7.} Fix $b > 1$, $y > 0$, and prove that there is a unique real $x$ such that $b^x = y$, by completing the following outline. (This $x$ is called the \textit{logarithm of $y$ to the base $b$}.)
	\begin{enumerate}
	...
	\clearpage
	\noindent
	\textbf{10.} Suppose $z = a + bi$, $w = u + iv$, and
	\[
	...
	\noindent
	\textbf{11.} If $z$ is a complex number, prove that there exists an $r \ge 0$ and a complex number $w$ with $|w| = 1$ such that $z = rw$. Are $w$ and $r$ always uniquely determined by $z$?
	...
	\noindent
	\textbf{14.} If $z$ is a complex number such that $|z| = 1$, that is, such that $z\bar{z} = 1$, compute
	\[
	|1 + z|^2 + |1 - z|^2.
	\]
	...
	\clearpage
	\noindent
	\textbf{16.} Suppose $k \ge 3$, $x, y \in \mathbb{R}^k$, $|x - y| = d > 0$, and $r > 0$. Prove:
	\begin{enumerate}
	...
	\noindent
	\textbf{18.} If $k \ge 2$ and $x \in \mathbb{R}^k$, prove that there exists $y \in \mathbb{R}^k$ such that $y \ne 0$ but $x \cdot y = 0$. Is this also true if $k = 1$?
	
	
	
	\noindent
	\textbf{19.} Suppose $a \in \mathbb{R}^k$, $b \in \mathbb{R}^k$. Find $c \in \mathbb{R}^k$ and $r > 0$ such that
	\[
	|x - a| = 2|x - b|
	\]
	if and only if
	...
	|x - c| = r.
	\]
	\textit{(Solution: }$3c = 4b - a,\; 3r = 2|b - a|.$\textit{)}
	
	\noindent
	\textbf{20.} With reference to the Appendix, suppose that property (III) were omitted from the definition of a cut.  
